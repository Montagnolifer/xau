...
class ImportShopeeApi extends BaseApiClient {
  constructor() {
  ...
  async listProducts(accountId: number): Promise<MarketplaceImportListResponse> {
    return this.authenticatedRequest<MarketplaceImportListResponse>(
      `/marketplace/shopee/accounts/${accountId}/products`,
      { method: 'GET' },
    )
  }
  ...
  async importProducts(
    ...
    return this.authenticatedRequest<ImportMarketplaceProductsResponse>(
      `/marketplace/shopee/accounts/${accountId}/import`,
      {
        method: 'POST',
...
export const importShopeeApi = new ImportShopeeApi()
